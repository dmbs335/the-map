...
| **TLD nameserver domain expiration** | A ccTLD's authoritative nameservers are themselves registered under that same TLD (e.g., `.io` NS records point to `ns-a1.io`, `ns-a2.io`, etc.). When one or more of these NS domain registrations expire or become available, an attacker registers the expired NS domain and gains authoritative control over a fraction of all DNS resolution for the entire TLD — enabling mass traffic interception, certificate misissuance, and credential theft for every domain under that TLD | TLD NS records reference domains within its own namespace; NS domain registration lapses or is purchasable; no registry lock on NS domains (The Hacker Blog, ".io" research, 2017) |
