...
| **IIS output cache query string parsing differential** | IIS Output Caching module's query string parser takes only the first occurrence when duplicate query string keys exist to compute the cache key, while the ASP.NET backend concatenates all values. This HTTP Parameter Pollution discrepancy enables cache poisoning: an attacker sends `?key=evil&key=normal`, IIS caches the response under the `key=evil` variant, but the backend processes `key=evil,normal` — subsequent legitimate `?key=evil` requests receive the poisoned response | CVE-2022-22040; Microsoft IIS with Output Caching enabled; ASP.NET backend using `Request.QueryString` |
| **IIS LKRHash token cache authentication bypass** | IIS uses LKRHash tables to cache authenticated session tokens. A copy-paste bug in the hash collision resolution code compares the username field twice instead of comparing username then password. When two entries collide in the hash bucket, an attacker can reuse another user's cached token with an arbitrary password — the comparison succeeds because both username checks pass while the password is never verified | CVE-2022-30209; Microsoft IIS with Windows authentication; exploitable on Exchange Server for passwordless account hijacking of monitoring service accounts |
...
| **Floating-point / number parser DoS** | Certain floating-point string representations (e.g., extremely long decimal expansions, subnormal numbers near `2.2250738585072012e-308`, or deeply nested scientific notation) trigger worst-case behavior in number parsing algorithms (`strtod`, `parseFloat`, `Double.parseDouble`), consuming disproportionate CPU per value. A single carefully crafted numeric string in a JSON field, query parameter, or form input can block a worker thread for seconds. The canonical Java case (CVE-2010-4476, `Double.parseDouble` infinite loop on `2.2250738585072012e-308`) and the parallel PHP `strtod` issue (CVE-2010-4645) were patched in major runtimes circa 2011; modern occurrences require either custom parsers or older/embedded runtimes | Application parses user-supplied numeric strings without input length limiting before parse; language-specific parser implementations with worst-case edge cases; runtimes predating the 2011 `parseDouble`/`strtod` fixes |
...
| Subtype | Mechanism | Key Condition |
|---|---|---|
| **Client-initiated renegotiation flood** | The client initiates TLS renegotiation repeatedly on an established connection. Each renegotiation requires the server to perform a full handshake (RSA key exchange, certificate chain validation) — consuming ~15x more CPU than the client. Tool: thc-ssl-dos. Per RFC 5746 / secure-renegotiation, mainstream stacks (OpenSSL ≥ 1.0.1, ~2012) default to disabled or rate-limited client-initiated renegotiation; live exposure is now confined to legacy/embedded TLS stacks or explicit re-enablement | Servers with client-initiated renegotiation enabled; no renegotiation rate limiting; legacy TLS stacks predating the post-CVE-2009-3555 fix |
