...
| **PDO Prepared Statement Null Byte** | PDO emulated prepares misparse null bytes (`\0`) at escape sequence boundaries, causing placeholder replacement to desynchronize and inject attacker-controlled SQL fragments into the query | Requires `PDO::ATTR_EMULATE_PREPARES = true` (default in some configurations); null byte in parameter value splits the escape context |
...
| §3-1 + §1-3 (MySQL RCE) | CVE-2025-25257 (Fortinet FortiWeb) | CVSS 9.6. Unauth SQLi → SELECT INTO OUTFILE → Python RCE. PoC published |
...
| §3-1 (Full-Text Search Oracle) | ReDisclosure Research (2025) | MySQL full-text boolean parsing abused to turn redirect-based endpoints into blind inference oracles; novel data extraction via `MATCH ... AGAINST` |
| §8-3 (PDO Null Byte) | slcyber.io Research (2025) | SQL injection via PDO prepared statement null byte misparsing; bypasses parameterized query protections in emulated prepare mode |
