...
|---|---|---|---|
| **Question mark glob** | `?` matches any single character. Used to spell out binaries from filesystem: `/???/??t /???/??????` → `/bin/cat /etc/passwd` | Linux | `/???/???/??????` → `/usr/bin/whoami` |
...
A class of vulnerabilities (CERT VU#123335) where §7-3 "Safe-by-Default APIs" become exploitable on Windows. When array-based process creation APIs invoke a `.bat` or `.cmd` file, Windows implicitly spawns `cmd.exe` to handle the batch file. Language runtimes escape arguments using backslash conventions (designed for `execvp()`), but `cmd.exe` uses caret (`^`) as its escape character — creating a parsing discrepancy that permits argument injection escalating to arbitrary command execution.

| Language | Affected API | Patch Status | CVE |
|---|---|---|---|
...
| **Python** | `subprocess.Popen(shell=False)` | Documentation update only | — |
| **Ruby** | `system("cmd", "arg")` (multi-arg) | Documentation update only | — |
| **Go** | `exec.Command` | Documentation update only | — |
| **Java** | `ProcessBuilder` | Won't fix | — |
| **Erlang** | `os:cmd` | Documentation update only | — |

**Exploitation condition:** The application must (1) run on Windows, (2) invoke a `.bat`/`.cmd` file — or omit the extension while a same-named `.bat` exists on `PATHEXT` — via an array-based API, and (3) pass user-controlled input as arguments. Even naive caret-escaping can be bypassed via `cmd.exe`'s `%CMDCMDLINE%` variable substring extraction, which reconstructs unescaped characters at runtime.
