...
| `01-injection/command-injection/` | OS shell command injection | Service protocol command injection (no shell involved) |
| `Uncovered_RCE_Vectors` §8 (referenced) | Redis MODULE LOAD, UAF, Lua sandbox escape as **RCE outcomes** | The **injection mechanism** — how protocol commands are injected in the first place |
| `01-injection/ldap-xpath/` | LDAP **query syntax** injection | Protocol-level framing; LDAP protocol briefly in §1-4 |
...
| Subtype | Mechanism | Key Condition |
|---|---|---|
| **MongoDB OP_COMPRESSED heap memory disclosure (MongoBleed)** | MongoDB's OP_COMPRESSED message format includes an `uncompressedSize` field. When an attacker sends a compressed message declaring a much larger uncompressed size than the actual payload, MongoDB allocates an oversized buffer, fills it with the small decompressed payload, and the rest remains as **uninitialized heap memory**. A malformed BSON document without a null terminator causes MongoDB to scan past the payload into heap memory, and when parsing fails, the server includes the leaked heap data in the error response (CVE-2025-14847, CVSS 9.1, CISA KEV) | MongoDB with compression enabled (default); unauthenticated access to port 27017; >213K exposed instances on Shodan |
...
| **Node.js http module CRLF** | Node.js `http.request()` historically allowed CRLF in various request components. The `unicode-characters-in-path` fix (CVE-2020-7610 and related) addressed Latin-1 range characters but specific bypass patterns persisted in certain versions | Affected Node.js versions; user-controlled URL path |
