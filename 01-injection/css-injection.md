...
| **Ad blocker filter rule exfiltration** | Ad blocker (uBlock Origin 등) CSS filter rules 자체를 side-channel로 활용 — 특정 attribute selector를 포함하는 cosmetic filter가 매칭될 때 발생하는 DOM 변형 또는 네트워크 요청 차이를 관찰하여 페이지 내 민감 데이터를 추론. 공격자는 malicious filter list를 배포하거나 기존 filter list에 injection하여, 사용자의 ad blocker가 CSS selector 기반 exfiltration을 자동으로 수행하도록 유도 | 피해자가 custom/malicious filter list를 구독하거나, filter list 배포 채널이 compromise된 경우; 대상 페이지에 예측 가능한 DOM 구조 존재 |
