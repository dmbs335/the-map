...
| **Script Injection** | Elasticsearch's Painless scripting language is sandboxed and blocks `java.lang.Runtime` access. Exploitation primarily targets **deprecated Groovy scripting** (Elasticsearch < 1.6 with dynamic scripting enabled). Public Painless sandbox-escape CVEs are scarce; treat any "Painless RCE" claim as requiring per-CVE verification (commonly cited IDs such as CVE-2021-22144 (Grok parser DoS) and CVE-2023-31419 (HTTP 1-byte over-read) are *not* Painless escapes). | Legacy Groovy: `{"script":"java.lang.Runtime.getRuntime().exec('cmd')"}` ; Painless is safe against direct RCE by design | Dynamic scripting enabled (legacy); or a verified Painless sandbox bypass CVE present |
...
Memcached uses a text-based protocol where commands are delimited by `\r\n`. When client libraries construct commands by concatenating user input without sanitization, CRLF injection enables arbitrary Memcached command execution.

| Subtype | Mechanism | Payload Example | Key Condition |
|---------|-----------|-----------------|---------------|
| **CRLF Command Injection** | Injecting `\r\n` into cache keys or values to terminate the current command and inject additional Memcached commands (`set`, `delete`, `flush_all`). | `key\r\nset injected 0 3600 5\r\nPWNED\r\n` | Client library does not sanitize CRLF in key/value parameters |
| **Session/Cache Poisoning** | Injecting `set` commands to overwrite cached session data or application cache entries with attacker-controlled values, enabling session hijacking or logic manipulation. | `\r\nset session:victim 0 3600 <len>\r\n<forged_session>\r\n` | Application uses Memcached for session storage; CRLF injectable |
| **CRC32 Collision Key Routing** | In libraries that use CRC32 hashing for consistent hashing across Memcached nodes (e.g., pylibmc), crafting cache keys whose CRC32 values collide enables targeting specific Memcached nodes. Combined with CRLF injection, the attacker controls both which node receives the command and what command is executed. | Key crafted to produce CRC32 collision with target node's hash range | Client uses CRC32-based consistent hashing (pylibmc, libmemcached); multi-node Memcached cluster |
...
| §4-3 ($mergeCursors authorization bypass) | CVE-2025-6713 (MongoDB Server < 8.0.7) | Unauthorized data access bypassing RBAC in sharded deployments. CVSS 7.7 per MongoDB (CNA, S:C); NVD analysis 6.5 (S:U). Note: This is a server-side authorization flaw, not a NoSQL injection — included here for completeness as it involves aggregation internals |
