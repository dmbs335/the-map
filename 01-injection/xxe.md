...
| **`file:////server/share`** | UNC path for SMB/CIFS access | Windows — triggers NTLM authentication to attacker's SMB server (§7-4); also serves as a data exfiltration channel when entity-resolved file content is embedded in the UNC path, sending data over SMB to an attacker-controlled share (§7-3) |
...
| **Windows UNC/SMB exfiltration** | §1-1 + §3-1 (UNC) + attacker SMB server | Yes (SMB data channel handles multi-line natively) | Yes (binary-safe transport; no URL encoding limitations) |
...
| External entities blocked (PHP) | Chain parameter-entity expansion with `php://filter` stream wrappers; in configurations where `libxml_disable_entity_loader` restricts external entity resolution but PHP stream wrapper handling remains active, parameter entities referencing `php://filter` chains can still trigger local file reads through the stream layer | §1-2, §3-4 |
