...
**QuicDraw: HTTP/3 QUIC FIN Burst Racing.** The single-packet attack principle extends naturally to HTTP/3 (QUIC): because QUIC multiplexes streams over UDP rather than TCP, an attacker can coalesce the final DATA frames (with FIN bit set) from multiple QUIC streams into a single UDP datagram. When the server processes this datagram, all stream completions are delivered in a single read event — achieving the same synchronization effect as the HTTP/2 single-packet technique but over an entirely different transport stack. The key parameter governing capacity is the server's `max_streams` transport parameter (negotiated during the QUIC handshake), which bounds the number of concurrent streams that can race within a single connection. Unlike HTTP/2 over TCP, QUIC's UDP-based framing eliminates TCP head-of-line blocking, potentially achieving tighter synchronization for high-stream-count races. As HTTP/3 deployment expands, defenders must account for QUIC-native race delivery mechanisms that bypass HTTP/2-specific mitigations.
...
| **Process state file race in embedded systems** | Embedded devices (routers, IoT gateways) use filesystem-based process state files (PID files, lock files, status flags) for inter-process coordination during firmware operations (update, reboot, service restart). An attacker triggers a legitimate administrative operation and races to modify state files during the window between the operation's validation and execution phases — e.g., replacing a process state file with a symlink to a writable configuration, or modifying a temporary firmware image between integrity check and flash write. The constrained runtime environment (single-threaded event loop, no ASLR, no sandboxing) amplifies exploitability compared to general-purpose OS targets | Embedded/IoT device with filesystem-based IPC; administrative operation triggerable remotely (web UI, TR-069, UPnP); no atomic file operations or advisory locking; predictable state file paths (NetModule Router CVE, 2024) |
...
### §8-5. Client-Side / Browser Race Conditions

Race conditions are not confined to server-side processing. The browser environment introduces its own concurrency surfaces where attacker-controlled timing manipulates client-side state transitions.

| Subtype | Mechanism | Key Condition |
|---------|-----------|---------------|
| **Connection pool exhaustion race** | Browsers enforce per-domain connection limits (typically 6 for HTTP/1.1). An attacker saturates the pool with long-running requests, then triggers a security-sensitive navigation or API call that blocks until a connection frees. During this stall, a concurrent DOM mutation (e.g., form target change, URL rewrite) alters the request's semantics before it dispatches | Victim page shares connection pool with attacker-influenced resources; HTTP/1.1 connection limit reached |
| **Digest cycle / framework re-render race** | Client-side frameworks with asynchronous rendering cycles (e.g., AngularJS digest, React concurrent mode) may re-evaluate template expressions or DOM bindings before sanitization or security annotations complete. An attacker triggers a state mutation during the framework's asynchronous update window, causing unsanitized data to be rendered momentarily | Framework with asynchronous DOM reconciliation; attacker can influence component state timing |
| **Framework initialization race (DOM injection)** | Attacker injects DOM content (via clipboard data or other DOM manipulation) during the window between framework bootstrap start and initialization completion. The framework (e.g., AngularJS) compiles the injected content as executable expressions upon initialization, achieving XSS through a timing gap | Client-side framework with deferred DOM compilation; attacker can inject DOM content before framework initialization completes (RyotaK, 2023) |
...
| 14 | CyberArk | QuicDraw: Taking the Single Packet Race Condition Attack to HTTP/3 | [CyberArk Research](https://www.cyberark.com/resources/threat-research-blog/quicdraw-taking-the-single-packet-race-condition-attack-to-http-3) | 2025 |
