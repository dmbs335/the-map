...
| Subtype | Mechanism | Key Condition |
|---|---|---|
| **Network MitM Cookie Setting** | An active network attacker intercepts an HTTP request to any subdomain and responds with `Set-Cookie` for the target domain. Modern browsers (implementing draft RFC 6265bis) generally block setting `Secure`-flagged cookies from non-secure (HTTP) origins, with localhost as a notable exception. However, the attacker can still set *non-Secure* cookies that may shadow or interfere with legitimate cookies. Legacy browsers that predate this restriction remain fully vulnerable. | Victim makes any HTTP request; attacker controls network; legacy browser or non-Secure cookie targeting |
...
| **JWT Cookie Vulnerabilities** | JWTs stored in cookies inherit all JWT attack vectors: `alg: none` bypass, HMAC/RSA algorithm confusion, key brute-forcing, claim manipulation. CVE-2022-23529 (disputed/withdrawn — requires attacker to already control the secret key object, making the threat model circular; CVE formally rejected by Auth0/Unit 42) demonstrated key object injection in the `jsonwebtoken` library where a malicious object with a crafted `toString()` can be passed as the key (separate from HMAC/RSA confusion). Misconfigured `decode()` vs `verify()` calls skip signature verification entirely. | JWT in cookie; weak secret; algorithm confusion; missing verification |
...
| Subtype | Mechanism | Key Condition |
|---|---|---|
| **Cookie Jar Overflow** | Browsers enforce per-domain cookie limits (typically ~50-180 cookies per schemeful site in Chrome/Firefox; Safari does not enforce any per-site cookie limit). Setting many new cookies forces eviction of existing cookies (oldest-first or LRU). An attacker sets enough cookies to evict a legitimate cookie, then replaces it with a malicious one. Safari's lack of limits makes eviction-based attacks impossible but leaves cookie bomb attacks unbounded. | Attacker can set cookies (subdomain XSS, cookie tossing); target cookie is evictable; eviction not available on Safari |
...
| **Cookie Ordering Ambiguity** | When multiple cookies with the same name exist (different Path/Domain), the order in which browsers send them is loosely specified. Safari sorts cookies by most-recent-first (creation time descending), while Chrome and Firefox sort oldest-first (creation time ascending, as mandated by RFC 6265bis). Servers that read only the first value may get the attacker's cookie instead of the legitimate one, and cookie shadowing attacks must be tailored to the target browser's ordering behavior. | Multiple same-name cookies; server reads first occurrence; browser-specific ordering |
...
| §8-2 (CORF token fixation) | CVE-2022-24895 (Symfony), CVE-2022-25896 (Passport), CVE-2022-29019 (Fastify), CVE-2022-35943 (Shield/CI4), CVE-2023-27495 (CI4 Shield) | CSRF bypass via token fixation in synchronizer token pattern; 9 of 13 frameworks vulnerable (Cookie Crumbles, 2023) |
