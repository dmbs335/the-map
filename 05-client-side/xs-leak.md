...
| **Ligature-Based Exfiltration (Fontleak)** | Uses font ligatures where multi-character sequences are rendered as single glyphs with deterministic widths. The **Fontleak** technique (2025) chains this with CSS container queries: the target text is placed in a container element, a custom ligature font assigns unique widths to character n-grams, and `@container` size queries trigger `url()` requests based on the container's rendered width — leaking the matched text sequence to the attacker without any JavaScript execution, purely through CSS rendering and font metrics | CSS injection; target text in a size-queryable container; works under strict CSP blocking all script execution |
