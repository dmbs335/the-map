...
| **Cloud Platform Same-Site Abuse** | Cloud platforms hosting multiple services under a shared domain (e.g., `*.azurewebsites.net`, `*.herokuapp.com`) create same-site relationships between unrelated tenants. EmojiDeploy (Ermetic, 2023) demonstrated this against Azure: CSRF on Azure's Kudu SCM management endpoint (`<app>.scm.azurewebsites.net`) was exploitable from any other `*.azurewebsites.net` site, enabling malicious code deployment (ZIP deploy) and achieving RCE on the victim's Azure App Service. | D6 | Cloud PaaS sharing a common eTLD+1 for tenant subdomains; management endpoints (Kudu SCM, admin panels) accessible on the shared domain |
