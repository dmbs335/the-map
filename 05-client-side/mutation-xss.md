...
| **Sanitizer API** (Browser Built-in) | mXSS prevention | Browser-native `setHTML()` avoids roundtrip entirely by building DOM directly; immune to mXSS by design. Historical allowlist gap (SVG `<use href>`, see §8-3) resolved — `<use>` now always removed. **Limited availability** (MDN, March 2026): Firefox 148+ ships `Element.setHTML()` unflagged in stable; Chrome 146+ exposes the Sanitizer API but `setHTML()` still requires `chrome://flags/#sanitizer-api` as of early 2026; Safari not supported; requires feature detection and fallback |
