...
| **CT log time-correlation attack** | Automatic certificate renewal systems (Let's Encrypt + cron, Plesk, cPanel, CyberPanel) issue certificates for all domains on the same server simultaneously. By querying CT logs (Censys `parsed.validity.start`, crt.sh sequential ID proximity) for certificates issued within seconds of a known domain's certificate, hidden co-hosted domains are discovered — even behind different registrars or Cloudflare proxies. When a CA omits exact timestamps (DigiCert, ZeroSSL set 00:00:00), the Signed Certificate Timestamp (SCT) field in the leaf certificate or sequential crt.sh ID proximity serves as a fallback correlation signal | Server renews certificates for multiple domains via automated batch job; CT logs record issuance timestamps or sequential order (PT Security, 2022 — PortSwigger Top 10 #1) |
