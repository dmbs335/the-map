...
| **SSRF via `request_uris`** | Registering `request_uris` or other request-object-by-reference locations that the AS later dereferences can become SSRF if those URIs point to internal resources. This is distinct from PAR itself, which does not let the client supply a fresh `request_uri` inside the pushed request | AS dereferences client-controlled request object URIs without network restrictions | D1 |
...
| ~~§10-3 (SAML assertion bypass)~~ | ~~CVE-2024-4985 (GitHub Enterprise Server)~~ | **Removed from OAuth mapping.** SAML-only vulnerability — encrypted assertion wrapping bypass. Sharing a product surface (GHES) does not make it an OAuth flaw. See `saml.md` S1-3 |
