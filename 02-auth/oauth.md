...
| **Authorization code injection via response replay** | Attacker captures a valid authorization response from a non-happy-path flow deviation — e.g., the AS returns a valid code on error paths, partial consent, or RFC-deviating edge cases — and replays the entire callback URL against the victim's browser session. Unlike login CSRF (which uses the attacker's own code) or simple code replay (which reuses a code at the token endpoint), this exploits AS behavioral deviations from the RFC that leak usable codes through unexpected response paths | AS returns valid codes on non-standard flow paths; code not bound to original browser session via PKCE | D3, D7 |
...
| **Browser-level redirect termination** | Browser and server resource limits create redirect-blocking primitives beyond JavaScript interception: protocol scheme rejection (`about:`, `data:` URIs → `about:blank#blocked`), dangling markup protection (Chrome blocks `<\t` in URLs), URL length overflow (>2MB), cookie bombing (~400KB cookies scoped to callback path → HTTP 431), WAF-triggering payloads in `state` parameter, and navigation rate limiting (>200 navigations/10s). Blocked redirect URLs persist in the Navigation API's `navigation.entries()`, readable cross-origin from the opener window (CVE-2022-4908; ref: [Johan Carlsson, "SOP bypass in Chrome using Navigation API"](https://joaxcar.com/blog/2023/10/06/cve-2022-4908-sop-bypass-in-chrome-using-navigation-api/)) | Attacker can trigger redirect to OAuth callback; browser enforces URL/navigation limits; Navigation API available (Chromium) | D3 |
| **Sandbox-based redirect blocking** | Opening the OAuth callback in a window spawned from a sandboxed iframe (`<iframe sandbox="allow-scripts allow-popups">`, without `allow-forms` or `allow-top-navigation`) blocks form-based redirects and navigation while maintaining script execution and same-origin API access. The authorization code remains in the pending URL, extractable by attacker script before the redirect completes | Attacker controls an iframe with sandbox attribute on a page in the redirect chain; sandbox permits scripts but restricts forms/navigation | D3 |
...
| §7-2 (Supply chain token compromise) | UNC6395/Drift-Salesforce (2025) | Single stolen OAuth token provided access to "over 700" customer Salesforce instances (Obsidian Security; corroborated by Google Threat Intelligence) |
| §2-1 (PKCE downgrade) | OAuth 2.1 (draft-ietf-oauth-v2-1-15, Internet-Draft as of 2026-03) | PKCE mandatory for all clients per draft specification; non-PKCE flows formally deprecated. Note: not yet a final RFC |
...
| §1-1 (Whitespace injection) | draw.io OAuth Token Leakage (CVE-2022-1774) | OAuth token leakage via whitespace bypass in redirect_uri validation (`https:// @evil.com/` userinfo trick). Ref: [huntr bounty](https://huntr.com/bounties/6ac07c49-bb7f-47b5-b361-33e6757b8757), [api-security.blog writeup](https://api-security.blog/2022/06/23/leakage-of-third-party-oauth-token-via-redirect/) |
| §1-1 (Path confusion) | ACSAC '23 "OAuth 2.0 Redirect URI Validation Falls Short, Literally" (Innocenti et al.) | redirect_uri validation bypass via URL parser differentials on path component interpretation. 6/16 IdPs failed validation; led to RFC 9700 §4.1.3 update |
...
A truly structural solution would require three properties that current OAuth deployments largely lack: **(1) sender-constrained tokens** (DPoP, mTLS-bound tokens) that cryptographically bind tokens to their intended holder, **(2) universal PKCE + issuer verification** (mandated in the OAuth 2.1 Internet-Draft (draft-ietf-oauth-v2-1-15; not yet a final RFC as of 2026-03)) that prevents code interception and mix-up attacks, and **(3) continuous authorization evaluation** that moves beyond one-time consent to real-time policy enforcement over token usage, scope consumption, and behavioral anomalies. Until all three are ubiquitous, the taxonomy above will continue to grow.
