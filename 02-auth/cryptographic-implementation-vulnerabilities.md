...
| **CBC Padding Oracle** | When a server reveals whether PKCS#7 padding is valid after decryption (via error messages, timing, or HTTP status codes), an attacker iteratively decrypts each byte by manipulating the preceding ciphertext block and observing the oracle response. Requires ~128 guesses per byte × 16 bytes per block ≈ 2,048 queries per AES block on average. | Server returns distinguishable responses for padding errors vs. application errors. Applies to any system decrypting CBC ciphertext from untrusted input. |
...
| **IV Reuse / Predictable IV** | Reusing the same IV with the same key produces identical ciphertext for identical first plaintext blocks, leaking equality. Predictable (e.g., chained / sequence-derived) IVs additionally enable chosen-plaintext attacks — the canonical example being **BEAST** (CVE-2011-3389) against TLS 1.0 CBC, where the IV for record N is the last ciphertext block of record N-1, letting a network-positioned attacker who can submit chosen plaintext to the same session guess preceding plaintext byte-by-byte. Note: BEAST is a *predictable / chained* IV flaw, not strict IV reuse; and the `C1 XOR C2 = P1 XOR P2` relation applies to CTR/stream ciphers, not CBC. | Static, chained, or otherwise predictable IV generation. Same key used across multiple encryptions. |
...
| **Invalid Curve Attack** | If the ECDH implementation does not validate that the peer's public key lies on the expected curve, an attacker submits a point on a weak curve with small subgroup order. The resulting shared secret leaks the private key modulo the subgroup order. Chinese Remainder Theorem combines multiple leaks for full recovery. Primarily an ECDH key exchange vulnerability — ECDSA verification is not directly affected by this attack pattern. | Missing point validation in ECDH key exchange. The implementation accepts arbitrary (x, y) coordinates without verifying curve membership. |
...
| **ROBOT (Return of Bleichenbacher's Oracle Threat)** | Modern variant using subtle side-channels as oracles: TCP reset vs. timeout, duplicated TLS alerts, HTTP status code differences. Public research found high-profile products and a measurable share of popular sites affected. | RSA key exchange enabled in TLS. Any observable behavioral difference on padding failure—even at the TCP level. |
| **Weak RSA Key Generation** | Keys generated with insufficient entropy, shared prime factors across devices (common in IoT/embedded), or degenerate parameters (e.g., `e=1`). Internet-wide studies have found remotely factorable RSA keys caused by shared prime factors. | Low-entropy PRNG at key generation time. Mass-produced devices with identical firmware generating keys at first boot. |
...
| Subtype | Mechanism | Key Condition |
|---|---|---|
| **Unvalidated Public Key Parameter (Chalkias Vulnerability)** | 45 Ed25519 libraries expose a "double public key" signing API (`sign(message, privateKey, publicKey)`) as an optimization parameter. If the library does not validate that the public key matches the private key, an attacker can invoke signing with a manipulated public key and directly recover the private key from the resulting signature (not via lattice reduction — the key is algebraically recoverable from a single signature with a mismatched public key). | Library exposes `sign(message, privateKey, publicKey)` API. No validation that `publicKey = privateKey * G`. Attacker can invoke signing with arbitrary public key values. |
...
| **Logjam (DH Export Downgrade)** | Attacker forces a TLS handshake to use 512-bit "export-grade" DH parameters, then performs Number Field Sieve precomputation to break the key exchange in real time. Public research found widespread reuse of weak export-grade groups. | TLS server supports DHE_EXPORT cipher suites. Client does not reject weak DH parameters. |
...
| Subtype | Mechanism | Key Condition |
|---|---|---|
| **Timing Attack on MAC Comparison** | Comparing MAC tags using standard string comparison (`==`) reveals the first differing byte through timing differences. An attacker iteratively brute-forces each byte by measuring comparison time, reducing the attack from 2^128 to 16×256 = 4,096 attempts for a 16-byte MAC. | Non-constant-time comparison function used for MAC verification. Measurable timing differences (local or network-adjacent). |
...
| Subtype | Mechanism | Key Condition |
|---|---|---|
| **bcrypt 72-Byte Truncation** | bcrypt silently truncates input to 72 bytes. Passwords longer than 72 bytes that share the same first 72 bytes produce identical hashes. Exploited in the Okta October 2024 incident: the cache key was a bcrypt hash of the concatenation (userId + username + password), and accounts with usernames ≥ 52 characters caused the combined input to exceed 72 bytes, effectively excluding the password portion from the hash. Multiple preconditions required: AD/LDAP delegated authentication, MFA not enforced, prior successful authentication cached, and authentication agent unavailable (triggering cache fallback). | Passwords or derived inputs exceeding 72 bytes passed to bcrypt. Cache key derived from bcrypt hash of concatenated fields. |
...
| Subtype | Mechanism | Key Condition |
|---|---|---|
| **Boot-Time Entropy Starvation** | Systems generating cryptographic keys immediately after boot (VMs, containers, IoT devices) may have insufficient entropy. `/dev/urandom` on Linux will return data even with low entropy, potentially producing predictable keys. Internet-wide studies found shared TLS/SSH keys caused by low-entropy key generation. | Key generation during early boot. VMs cloned from a single snapshot (identical initial PRNG state). Embedded devices with no entropy sources. |
...
| **WebCrypto API Misuse** | The W3C WebCrypto API (`SubtleCrypto`) provides low-level cryptographic primitives in the browser. Its API design requires developers to make correct mode/parameter choices. Common misuses: using `AES-CBC` without separate HMAC, specifying SHA-1 as the hash algorithm when generating/importing RSA-OAEP keys (`RsaHashedKeyGenParams.hash`), or implementing custom ECDSA nonce generation outside `SubtleCrypto.sign()` (which handles nonces internally — the risk is in non-WebCrypto JS ECDSA libraries that use `Math.random()`). | Browser-based E2EE implementation. Developers unfamiliar with cryptographic primitive selection. No high-level crypto library wrapper. |

### §9-3. Protocol-Level E2EE Weaknesses

| Subtype | Mechanism | Key Condition |
|---|---|---|
| **File Injection / Tampering** | E2EE cloud storage services that don't authenticate file metadata allow the server to inject or tamper with files. Per ETH Zurich (2024): Sync and pCloud — targeted file injection via unauthenticated metadata; Tresorit — key replacement and metadata tampering (not direct file injection); Seafile — fixed IV enabling content leakage (see §9-2). Each provider has distinct attack surfaces. | File metadata (name, path, sharing info) not cryptographically bound to encryption key. Server controls file listing. |
...
| §2-1 (ECDSA timing) | CVE-2024-13176 (OpenSSL) | ~300ns timing signal on ECDSA nonce inversion. Affects OpenSSL 3.4.0, 3.3.x, 3.2.x, 3.1.x, 3.0.x, 1.1.1, and 1.0.2 (patched in 3.4.1, 3.3.3, 3.2.4, 3.1.8, 3.0.16, 1.1.1zb, 1.0.2zl). |
| §2-1 (ECDSA timing, Minerva) | CVE-2024-23342 (python-ecdsa) | Nonce bit-length leakage via signing time. Private key recoverable. |
| §2-1 (ECDSA timing, SM2) | CVE-2025-9231 (OpenSSL ARM64) | SM2 signing timing variations on 64-bit ARM. Affects OpenSSL 3.5.x, 3.4.x, 3.3.x, 3.2.x (patched in 3.5.4, 3.4.3, 3.3.5, 3.2.6). Versions 3.1.x and 3.0.x are not affected. |
| §2-2 (RSA padding oracle) | ROBOT (2017, ongoing) | High-profile products and a measurable share of popular sites were vulnerable. (See `tls-security.md` §2-1 for TLS context.) |
| §6-3 (CMS IV overflow) | CVE-2025-15467 (OpenSSL 3.x) | Pre-auth stack buffer overflow in CMS parsing. RCE achieved by JFrog. |
| §6-3 (multiple parser vulns) | 12 CVEs (OpenSSL, 2026-01-27 disclosure, discovered by AISLE Research) | Multiple vulnerabilities across QUIC, PKCS#12, CMS, TLS 1.3, BIO subsystems. Mix of HIGH/MODERATE/LOW severity. |
| §3-2 (MD5 collision) | CVE-2024-3596 (BlastRADIUS) | RADIUS Response-Authenticator forgery via MD5 chosen-prefix collision. |
| §3-4 (bcrypt truncation) | Okta incident (Oct 2024) | Cache key collision via bcrypt 72-byte truncation. Authentication bypass when username ≥ 52 chars, AD/LDAP delegated auth, MFA not enforced, cached prior auth, and agent unavailable. |
| §9-2 (E2EE implementation) | ETH Zurich (2024) | Per-service findings: Seafile — fixed IV in AES-CBC file chunk encryption; Sync, pCloud — targeted file injection via unauthenticated metadata; Tresorit — key replacement and metadata tampering; Icedrive — unauthenticated CBC encryption. Each service has distinct vulnerability profiles. |
| §1-1 (CBC padding oracle) | CVE-2021-31196 (Microsoft Exchange) | ProxyOracle: AES-CBC padding oracle on Exchange FBA cookies. Chained with reflected XSS (CVE-2021-31195) to steal HttpOnly cookies via SSRF redirect, then offline decryption recovers plaintext credentials. First 12 bytes predictable (`Basic ` in UTF-16), eliminating IV recovery. |
