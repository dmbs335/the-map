...
| Subtype | Mechanism | Key Condition |
|---|---|---|
| **bcrypt 72-Byte Truncation** | bcrypt silently truncates input to 72 bytes. Passwords longer than 72 bytes that share the same first 72 bytes produce identical hashes. Exploited in the Okta October 2024 incident: the cache key was a bcrypt hash of the concatenation (userId + username + password), and accounts with very long usernames caused the combined input to exceed 72 bytes, effectively excluding the password portion from the hash — allowing authentication with any password. | Passwords or derived inputs exceeding 72 bytes passed to bcrypt. Cache key derived from bcrypt hash of concatenated fields. |
