...
| **Double evaluation** | Parameter value is resolved once by the binder, then the result is re-evaluated as an expression (S2-061, CVE-2020-17530). Note: S2-045 (CVE-2017-5638) is a distinct vulnerability — OGNL injection via Content-Type header in the Jakarta Multipart parser, not a data binding double evaluation issue. | Error messages or intermediate values containing user input are re-parsed |
