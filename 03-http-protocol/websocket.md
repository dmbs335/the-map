...
| **Reconnection Token Interception** | When a WebSocket connection drops, the client library automatically reconnects and re-authenticates by transmitting the session token (via URL query parameter, first message, or custom header). An attacker triggers disconnection via cross-origin `postMessage` (e.g., sending a crafted message that the client-side handler interprets as a disconnect command), then intercepts the token during the reconnect handshake — through Referer leakage if the token is in the URL, cross-origin message broadcast if the reconnect logic emits auth state, or timing-based observation of the reconnect endpoint | Client re-transmits auth credentials during reconnection in a less-secure channel than the original connection (token in URL, postMessage broadcast without targetOrigin); reconnection can be triggered externally via cross-origin messaging |
