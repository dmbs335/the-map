...
0.CL typically combines with CL header whitespace obfuscation (`Content-Length : 76` with a space before the colon, see §3) to prevent the Front-End from recognizing the CL header. As of 2025, 0.CL has been confirmed at scale against Akamai (CVE-2025-32094), Cloudflare (H2.0 variant exposing 24M websites, $7K bounty), and Netlify. Treat large bounty/report totals from the broader 2025 desync research campaign as campaign-level context, not as figures tied solely to CVE-2025-32094.
...
**Guided H2 Conversion Fuzzing.** Follow-up guided fuzzing extended the grammar-based approach to 16 proxy technologies (11 standalone proxies + 5 CDNs: Akamai, Cloudflare, Azure Front Door, Fastly, AWS CloudFront), discovering 39 distinct anomaly types across 66 unique proxy-anomaly pairs — 50% more anomalies than the grammar-based predecessor. Six systematic input categories drive these anomalies:

| Input Category | Anomaly Pairs | Mechanism |
|---|---|---|
...
| **Malformed chunked → CL fallback** | `Transfer-Encoding: chunked` with an invalid chunked body (missing terminator, bad chunk sizes) | Akamai falls back to `Content-Length` when the chunked body is malformed — if the origin processes the malformed chunked body differently, the CL fallback creates a framing mismatch. Tomcat rejects chunked transfer encoding entirely in HTTP/1.0 requests |
...
| Technique | Mutation Example | Parser Difference |
|---|---|---|
| **Colon whitespace** | `Content-Length : 42` | Core enabler for 0.CL attacks: Front-End fails to recognize the CL header → treats body as zero; Back-End recognizes it → reads 42 bytes. This mutation was part of the Akamai CVE-2025-32094 attack class; public bounty totals should not be attributed solely to this CVE unless the source explicitly does so |
...
| **:authority vs Host divergence** | When both `:authority` pseudo-header and `Host` header are present with conflicting values, proxies disagree on which to use for routing after downgrade — some prefer `:authority`, others prefer `Host` | Enables host-header-based smuggling and routing confusion in H2→H1 chains |
...
| **Request confusing** | Proxy and origin disagree on whether an incoming byte sequence constitutes a valid request — one accepts it as a request while the other ignores it, desynchronizing the request-response pairing on the connection | Enables request smuggling without traditional CL/TE conflicts; distinct from body-length-based desync |
...
Gray-box coverage-directed differential testing across 19 implementations extending desync discovery to HTTP responses and CGI responses systematically identified these classes through five discrepancy categories: (1) non-standard number parsing, (2) trailer section acceptance divergence, (3) non-standard line separator handling, (4) request-side TE.CL disagreement, and (5) response-side TE.CL disagreement. This resulted in 9 CVEs across multiple vendors (including request smuggling, request confusing, response forgery, and response stealing vulnerabilities).
...
|---|---|---|
| §1-2 (0.CL) + §3 (CL whitespace) + Early Response Gadget + §2-2 (line folding) | CVE-2025-32094 (Akamai) | Akamai Ghost request smuggling via crafted traffic. Tens of millions of sites affected is a campaign-level claim from public research; avoid treating broader bounty/report totals as CVE-specific unless independently sourced |
...
| §8-3 (semantic gap — interpretation) | CVE-2019-17569, CVE-2020-1935 (Apache Tomcat) | CL+TE prioritization and malformed TE handling discrepancies in Tomcat's HTTP connector |
| §8-3 (semantic gap — interpretation) | CVE-2020-0645 (Microsoft IIS) | Header parsing leniency mismatch exploitable in proxy-IIS pairs |
| §8-3 (semantic gap — prioritization) | CVE-2020-2867, CVE-2020-14588, CVE-2020-14589 (Oracle Weblogic) | CL vs TE priority conflicts and malformed header tolerance in Weblogic's HTTP parser |
| §8-3 (semantic gap — tolerance) | CVE-2020-1944 (Apache Traffic Server) | Request parsing tolerance gap exploitable across 29 server pairs |
| §8-4 (request + response + CGI parsing) | 9 CVEs across Apache, Squid, ATS, Twisted, gevent, Tomcat, Jetty, Falcon (HDHunter) | 17 desync vulnerabilities in 19 implementations. Per-product: Squid (number parsing, critical), ATS (trailer section, critical), gevent (trailer section, critical), Tomcat (trailer section, high, $4,660 bounty), Apache (response TE.CL, low), Twisted (response ordering + response TE.CL, high), Jetty (number parsing, moderate), Falcon (number parsing, moderate). 5 discrepancy categories: non-standard number parsing, trailer section acceptance, non-standard line separator, request TE.CL handling, response TE.CL handling |
...
| **H2FUZZ** | H2→H1 conversion in 16 proxy technologies (11 standalone + 5 CDNs: Akamai, Cloudflare, Azure Front Door, Fastly, AWS CloudFront) | Guided HTTP/2 frame fuzzer — extends FRAMESHIFTER with feedback-driven mutation, discovering 39 anomaly types across 66 unique proxy-anomaly pairs (50% more than FRAMESHIFTER). Six input categories: Missing END_STREAM, No Mutation Filter, No Mismatch Check, Mixed :authority/Host, Flawed Duplicate Validation, Pad Length Overflow | RAID 2025 |
| **HDiff** | 10 HTTP implementations (Apache, Nginx, Tomcat, Weblogic, IIS, Squid, ATS, HAProxy, Caddy, Lighttpd) | Semi-automatic: NLP extracts 117 Specification Rules + 269 ABNF rules from RFC documents → generates 101,085 targeted test cases → differential testing. Specification-driven rather than grammar-driven. DSN 2022 Best Paper Runner-Up | DSN 2022 |
| **HTTP Garden** | 51 implementations (32 origin servers + 12 local transducers + 7 cloud transducers; including Aiohttp, Apache, Bun, CherryPy, Daphne, Deno, fasthttp, Go, Gunicorn, H2O, Hyper, Hypercorn, Jetty, libevent, libsoup, Lighttpd, Mongoose, Nginx, Node.js, OpenLiteSpeed, OpenBSD httpd, Passenger, Puma, Tomcat, Tornado, uhttpd, Unicorn, Uvicorn, Waitress, WEBrick, Werkzeug + proxies) | Coverage-guided differential fuzzing of HTTP/1.1 *request streams* (not individual requests). Fine path δ-diversity metric for parent selection. Interactive REPL for discrepancy exploration. Found 122 discrepancies, 39 exploitable, 68 patches | DEF CON 31 / 2023 |
...
| **HDHunter** | 19 HTTP implementations: NGINX, Apache, Lighttpd, H2O, HAProxy, Squid, Varnish, ATS, Twisted, Tornado, gevent, Eventlet, Tomcat, Jetty, Puma, Falcon, uWSGI, Waitress, Gunicorn | Gray-box coverage-directed differential testing. First tool to systematically discover *response* and *CGI response* parsing discrepancies alongside request desync. Defines 4 attack types: request smuggling, **request confusing**, response stealing, response forgery. 5 discrepancy categories: non-standard number parsing, trailer section acceptance, non-standard line separator, request TE.CL handling, response TE.CL handling. 17 vulnerabilities, 9 CVEs | USENIX Security 2025 |
...
- James Kettle — *HTTP/1.1 Must Die: The Desync Endgame* (Black Hat USA 2025 / DEF CON 33). 0.CL with Early Response Gadgets, double-desync (0.CL→CL.0 conversion), Expect-based desync (vanilla and obfuscated), V-H/H-V discrepancy detection methodology, HTTP Request Smuggler v3.0. $350K+ bounties across multiple programs including Akamai (CVE-2025-32094), Cloudflare (24M websites, $7K), Netlify, T-Mobile ($12K), GitLab ($7K), LastPass ($5K), EXNESS ($7.5K). All bounties donated to charity. Published `http1mustdie.com`; do not conflate program-wide figures with a single CVE without a source that makes that mapping.
...
- Anthony Gavazzi, Weixin Kong, Engin Kirda — *H2FUZZ: Guided, Black-box, Differential Fuzzing for HTTP/2-to-HTTP/1 Conversion Anomalies* (RAID 2025). Follow-up to FRAMESHIFTER; guided fuzzing with comprehensive mutator and novel feedback system across 11 standalone reverse proxies and 5 CDNs (16 technologies). Discovered 39 anomaly types in 66 unique proxy-anomaly pairs — 50% more anomalies than FRAMESHIFTER. Six input categories: missing END_STREAM flag, no mutation filter, no mismatch check, mixed :authority/Host, flawed duplicate validation (OpenLiteSpeed), pad length overflow (Akamai).
...
- Ben Kallus, Prashant Anantharaman, Michael E. Locasto, Sean W. Smith — *The HTTP Garden: Discovering Parsing Vulnerabilities in HTTP/1.1 Implementations by Differential Fuzzing of Request Streams* (DEF CON 31 / 2023). Coverage-guided stream-level differential fuzzing across 27 origin servers and 12 proxies (39 total; GitHub repository has since expanded to 51 implementations); 122 discrepancies, 39 exploitable, 68 patches. Key findings: LiteSpeed octal CL, Python digit separators, Node.js bare CR, Puma two-byte terminator, ATS `0_ff` chunk parsing. Interactive REPL for discrepancy development. GitHub: `narfindustries/http-garden`.
...
| CVE-2025-32094 | Akamai CDN | 0.CL + CL whitespace obfuscation + obsolete line folding + Expect variants. Public desync research gives broad campaign and bounty context, but the exact bounty/report totals should not be treated as CVE-specific without primary-source attribution |
