...
| **HTTP/2 Smuggling (CL/TE Variants)** | Use headers like `transfer-encoding : chunked` (space before colon). RFC 7230 §3.2.4 requires servers to reject and proxies to strip whitespace between field-name and colon, but in practice many parsers are lenient and disagree: one peer treats the malformed name as invalid (and ignores the header), the other normalizes/accepts it as `Transfer-Encoding`. The smuggling primitive comes from this **parser-leniency desync** (one side sees TE, the other sees only CL), not from the RFC's TE-over-CL precedence rule per se. Common when an HTTP/2 frontend downgrades to HTTP/1.1 toward the backend. | Frontend (Cloudflare, nginx) and backend disagree on whether to accept/normalize/reject malformed `transfer-encoding` header names. |
...
| **Binary Metadata Encoding Bypass** | Binary metadata values use keys ending with `-bin` suffix. Per the gRPC HTTP/2 protocol spec (PROTOCOL-HTTP2.md), `-bin`-suffixed metadata MUST be base64-encoded on the wire (RFC 4648 §4; padded values must be accepted, un-padded should be emitted) — runtime libraries detect the suffix and apply encode/decode automatically. Attacks craft metadata keys without `-bin` but containing binary data, or send `-bin` keys with malformed/un-padded base64, or rely on inconsistent enforcement between the gateway and downstream service. Can trigger parser errors or bypass validation. | Implementation doesn't strictly enforce `-bin` suffix convention or base64 encoding expectations. |
