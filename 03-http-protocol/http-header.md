...
| **CSP Injection/Override** | Inject `%0d%0aContent-Security-Policy: default-src *` — per spec, multiple CSP headers are intersected (most restrictive wins), so this only works when no prior CSP header exists. If an existing strict CSP is present, injecting a permissive one has no effect in spec-compliant browsers | CRLF injection point + no existing CSP header, or non-spec-compliant CSP implementation |
...
| **Nested Response Splitting with TE Override** | Variant of the nested CSP bypass that injects `Transfer-Encoding: chunked` instead of manipulating `Content-Length`. Per RFC 9112, `Transfer-Encoding` takes precedence over `Content-Length` when both are present — the injected TE header forces the browser to use chunked framing, giving the attacker precise control over how much of the response body is parsed as HTML/JavaScript regardless of the original `Content-Length` value. More reliable than Content-Length truncation when the original response length is unpredictable or fixed | CRLF injection on same endpoint; CSP with `script-src 'self'`; browser honors TE/CL precedence on responses; server does not strip injected `Transfer-Encoding` header |
...
| **CSP Header Duplication** | Inject a second `Content-Security-Policy` header with permissive policy — per CSP Level 3 spec, multiple CSP headers are enforced as the intersection (most restrictive combination), so a more permissive duplicate cannot weaken the original policy. Exploitation requires non-spec-compliant browsers or injecting a **more restrictive** policy to cause DoS/functional breakage | CRLF injection point in response headers; target is functional disruption (not policy weakening) unless browser deviates from spec |
| **CSP Downgrade via Meta Tag** | If CRLF injection allows body injection, insert `<meta http-equiv="Content-Security-Policy" content="...">`. Per spec, meta CSP can only **add** restrictions (intersects with header CSP), not remove them. This technique only works when no header CSP exists, making the meta tag the sole policy | Body injection possible; **no existing header-based CSP** (meta CSP cannot weaken header CSP in spec-compliant browsers) |
