...
**Historical Impact:** The IIS Unicode directory traversal vulnerability (CVE-2000-0884) exploited overlong encodings of `..` and `/` to access arbitrary files. The Nimda worm used this vector for propagation. (Code Red II exploited a separate vulnerability — CVE-2001-0500, a buffer overflow in IIS Index Server's idq.dll.)
...
| **MySQL `utf8` vs. `utf8mb4`** | MySQL's `utf8` (alias `utf8mb3`) encoding only supports 3-byte UTF-8 (BMP only). 4-byte characters (emoji, supplementary planes) are unsupported: in strict SQL mode the insert is rejected with `ERROR 1366: Incorrect string value`; in non-strict mode the value is silently **truncated at the first 4-byte sequence** and a warning is emitted. The specific behavior is sql_mode-dependent (rejection vs. truncation) rather than U+FFFD/`?` substitution | MySQL table uses `utf8` instead of `utf8mb4` |
