...
| **DKIM Key Reuse / Weak Key** | Short DKIM keys (512-bit) are commodity-factorable on a single workstation. RSA-768 was factored once in 2009 by an academic team at the cost of ~2000 CPU-years — feasible for well-resourced attackers but not commodity. Either size enables an attacker who recovers the private key to forge DKIM signatures for the target domain. | Domain uses a DKIM key shorter than 1024 bits |
