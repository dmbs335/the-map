...
| **.NET BinaryFormatter** | Microsoft-deprecated binary formatter that invokes deserialization callbacks; obsolete-as-error in .NET 7 (compile-time SYSLIB0011), throws `NotSupportedException` at runtime by default in .NET 8 (opt-in via `EnableUnsafeBinaryFormatterSerialization`), implementation removed in .NET 9 (compatibility switch no longer available); cannot be secured and should never be used with untrusted data | Legacy .NET application still using `BinaryFormatter` |
...
| **JShell API + Reflection Bypass** | On modern JDK (17+), the JShell API (`jdk.jshell`) provides a legitimate code evaluation surface that bypasses both module access restrictions and JEP 290 filter patterns targeting traditional gadget classes. Combined with residual reflection capabilities in non-restricted packages, attackers construct chains that eval arbitrary code without touching blocked internal classes | JDK with `jdk.jshell` module available; JEP 290 filter configured with blocklist (not allowlist) |
...
| **Fastjson Autotype Bypass Evolution** | Fastjson `@type` triggers arbitrary type instantiation. Autotype blacklist bypassed across versions: `L`-prefix/`;`-suffix class name decoration (pre-1.2.42), double-encoding and cache mapping abuse (pre-1.2.48), `expectClass` parameter differential where inner types bypass the blocklist (1.2.68), exception-based autotype in safeMode (pre-1.2.83). Primary gadgets: `JdbcRowSetImpl` → JNDI, `BasicDataSource`, `JndiConverter`, `UnpooledDataSource` | Fastjson ≤ 1.2.83 without `safeMode=true`; gadget class on classpath |
...
| **PyYAML Code Execution** | `yaml.load()` with unsafe default `Loader` (pre-5.1 silently used the unsafe full loader; 5.1 still defaults to `FullLoader` but emits `YAMLLoadWarning` if no `Loader=` is supplied; 6.0+ makes the `Loader` argument mandatory). Independently, specific `FullLoader` bypasses were patched in point releases across 5.1–5.4. Processing `!!python/object/apply` tags enables arbitrary function calls | Unsafe YAML loader on untrusted input |
...
| **Gem::SafeMarshal Bypass** | Ruby's `Gem::SafeMarshal` wrapper restricts `Marshal.load()` to a subset of safe types. The `Time` class, permitted by SafeMarshal, serves as a deserialization primitive — its `_load` method invokes `rb_marshal_load` on attacker-controlled bytes, escaping from the safe-deserialization allowlist into unrestricted `Marshal.load`, from which a gadget chain can achieve RCE (GHSA-9p2x-9p6h-26mp / "SafeMarshal escape") | Application or RubyGems package manager using `Gem::SafeMarshal` for untrusted input |
