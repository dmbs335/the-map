...
| **Session Cookie Deserialization** | When `CookieStore` is used as session storage and the `secret_key_base` / `secret_token` is known or leaked, an attacker can forge session cookies containing malicious Marshal payloads that the server deserializes. Note: Rails 4.1+ generated new apps with `cookies_serializer = :json`; this attack requires Marshal serializer (legacy apps upgraded from pre-4.1, or apps explicitly configured to `:marshal` / `:hybrid`) | Known secret_key_base; CookieStore session backend; Marshal serializer |
...
| §1-1 + §1-4 (Marshal + Session Cookie) | `exploit/multi/http/rails_secret_deserialization` | RCE via forged session cookie when `secret_token` is known. This Metasploit technique is separate from CVE-2013-0156, which covers XML parameter parsing/object injection, and from CVE-2013-0277, which covers Active Record serialized attributes. |
...
| §1-1 (Predictable Secret in Dev Mode) | CVE-2019-5420 (Railties) | RCE via predictable `secret_key_base` in development mode — attacker derives the secret from the application name, forges session cookie with Marshal payload. Requires Marshal cookie serializer; note that new apps generated by Rails ≥ 4.1.0 (April 2014) default the `cookies_serializer` to `:json`, so this is primarily exploitable against legacy apps generated pre-4.1 (or apps that explicitly opt back into `:marshal`/`:hybrid`). CVSS 9.8 |
...
| **Metasploit** (Exploit Framework) | Rails session deserialization, path traversal, DoS | Separate exploit modules for CVE-2013-0156, CVE-2019-5418/5420, and session cookie forgery |
