...
| **Exposed environment files (`.env`)** | Application environment file (`.env`, `.env.production`, `.env.local`) served from the web root, exposing database credentials, API keys, and third-party service secrets | Dotfile deployed to web-accessible directory; no access restriction configured | D6, D2 |
| **Exposed `.git` directory** | `.git/` directory served from the web root, enabling full source code reconstruction via sequential object download — exposes credentials, API keys, and internal logic in commit history | Version control directory deployed to production web root; no access restriction on dotfiles | D6, D2 |
